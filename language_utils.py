...
class LanguageUtils:
    ...
    def __init__(self):
        ...
        self.remove_phrases_pattern = re.compile(r'\b(what is|calculate|the|of|please|compute|find|value|result)\b', re.IGNORECASE)
    ...
    def convert_to_radians(self, angle: str) -> float:
        ...
        if match:
            ...
            try:
                ...
                raise ValueError(f"Angle value '{angle_value}' cannot be converted to a float.")
            ...
            try:
                ...
                raise ValueError(f"Angle value '{angle}' cannot be converted to a float.")
    ...
    def convert_to_math(self, text: str) -> str:
        try:
            ...
            for keyword, pattern in self.functions_patterns.items():
                if keyword in text:
                    if keyword in ["sin", "cos", "tan"]:  # Handle trigonometric functions separately
                        ...
                        text = re.sub(pattern, lambda match: self._replace_function(match, keyword, angle), text, flags=re.IGNORECASE)
                    else:
                        text = re.sub(pattern, lambda match: self._replace_function(match, keyword), text, flags=re.IGNORECASE)
            ...
            if not text:
                raise ValueError("Empty math expression after conversion. Check the input for unsupported phrases.")
            
            # Check if the expression is valid
            try:
                eval(text, {"__builtins__": None}, {"sqrt": math.sqrt, "log": math.log, "sin": math.sin, "cos": math.cos, "tan": math.tan, "math": math, "abs": abs})
            except Exception as e:
                raise ValueError(f"Invalid math expression after conversion: '{text}'. Error: {e}")
            
            return text
        except Exception as e:
            raise ValueError(f"Error converting natural language to math expression: {e}. Input: '{text}'")
...
if __name__ == "__main__":
    ...
    assert language_utils.convert_to_math("Calculate the square root of 25") == "sqrt(25)"
    assert language_utils.convert_to_math("What is 5 to the power of 3?") == "5 ** 3"
    assert language_utils.convert_to_math("What is the factorial of 5?") == "math.factorial(5)"
    assert language_utils.convert_to_math("What is the absolute value of -7?") == "abs(-7)"
    assert language_utils.convert_to_math("What is the cosine of 60 degrees?") == f"cos({math.radians(60)})"
    
    # Test convert_to_radians
    print("\nTesting convert_to_radians...")
    assert math.isclose(language_utils.convert_to_radians("45 degrees"), math.radians(45))
